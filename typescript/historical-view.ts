...
const extract = document.querySelector(".registro-transacoes")! as HTMLElement;

function renderExtract() {
	const historical: Transaction[] = getHistorical();
	let viewHistorical: string = "";
	...
	if (historical[0]?.date) {
		...
		historical.forEach((trans, i, arr) => {
			if (i > 0) {
				const currentMonth: string = verifyMonth(trans.date);
				const lastMonth: string = verifyMonth(arr[i - 1]!.date);
...
function verifyMonth(date: string): string {
